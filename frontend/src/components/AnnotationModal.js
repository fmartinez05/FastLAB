...
const AnnotationModal = ({ step, onSave, onCancel }) => {
  ...
  const handlePointerDown = (e) => {
    ...
    setCurrentPoints([{ x: pos.x, y: pos.y, pressure: 0.5 }]);
  };

  const handlePointerMove = (e) => {
    if (!isDrawing.current || tool === 'eraser') return;
    const pos = e.target.getStage().getPointerPosition();
    setCurrentPoints(points => [...points, { x: pos.x, y: pos.y, pressure: 0.5 }]);
  };
  ...
  const currentPath = currentPoints.length > 0 ? <Path data={getSvgPathFromStroke(getStroke(currentPoints, currentPathOptions))} fill={tool === 'pen' ? 'black' : '#FFD700'} opacity={tool === 'highlighter' ? 0.5 : 1} globalCompositeOperation={tool === 'highlighter' ? 'multiply' : 'source-over'} /> : null;
